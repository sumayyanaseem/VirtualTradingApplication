...
public class FlexiblePortfolioImpl extends AbstractPortfolio {
  ...
  public void buyStocks(String companyName,
    ...
    Stock s = new Stock(cName, q, 0.0, action,
            0.0, date);
    if (stockMap.isEmpty()) {
      ...
      if (!m.containsKey(companyName)) {
        ...
        for (int i = 0; i < stockList.size(); i++) {
          ...
          if (areDatesEqual(dateBought, date)) {
            ...
            s = new Stock(companyName, totQty,
                    0.0, action, 0.0, date);
            stockList.remove(i);
            stockList.add(s);
  ...
  private void updatePortfolioHelper(String companyName, String quantity, String date,
  ...
  @Override
  public void updatePortfolioUsingFilePath(String filePath,
    ...
    parser.appendIntoFileUsingFilePath(
            filePath, portfolioName, companyName, quantity, action, date);
  }
        ...
        for (Stock listOfStock : listOfStocks) {
          ...
          if (listOfStock.getAction().equals("buy") && dateBoughtObj.compareTo(givenDateObj) <= 0) {
            ...
            totalCostBasis = totalCostBasis
                    ...
                    listOfStock.getCompanyTickerSymbol(), listOfStock.getQty(), date);
          }
        }
      }
    }
  ...
  private void validateInputsForSell(String quantity, String date) {
    //validation on companyName and
    // date chronological order is already handled in sellStocks method.
    validateQuantity(quantity);
    validateDate(date);
  }
  ...
  private void validateInputsForUpdate(String portfolioName,
                                       String companyName,
                                       String quantity, String date, String action) {
